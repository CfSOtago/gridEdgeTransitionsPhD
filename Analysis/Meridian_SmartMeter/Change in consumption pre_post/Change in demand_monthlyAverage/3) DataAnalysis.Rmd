...
meanDT <- meanDT[, .(meankW = mean(kW, na.rm = TRUE)), keyby =.(month, SolarPeriod, iD)] # Create mean for kW across dataset iDs for each half hour and SolarPeriod
...
myPlot <- ggplot2::ggplot(meanDT, aes(x=month, y=diffper,  group=month)) +
  geom_boxplot(outlier.shape=NA) +
   labs(x="Month",y="Reduction in demand (%)",title="Mean monthly demand reduction after solar installation across iDs")+
