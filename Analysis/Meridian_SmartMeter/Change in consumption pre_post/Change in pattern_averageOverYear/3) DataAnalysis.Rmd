...
meanDT <- meanDT[, .(meankW = mean(kW, na.rm = TRUE)), keyby =.(obsHalfHour, SolarPeriod)] # Create mean for kW across dataset iDs for each half hour and SolarPeriod
...
myPlot <- ggplot2::ggplot(meanDT, aes(x=obsHalfHour, y=meankW, colour=SolarPeriod)) +
  geom_line() +
  geom_point()+
   labs(x="Time of Day",y="Demand (kW)",title="Mean demand across years and iDs for pre/post solar installation")+
