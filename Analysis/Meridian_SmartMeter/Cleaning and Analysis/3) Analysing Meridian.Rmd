...
chosen <- sample(unique(impactSuPreDT$iD), 25)
...
chosen <- sample(unique(impactWiPreDT$iD), 25)
...
impactSuPreDT <- impactSuPreDT[, . (HhkWh = sum(kWh, na.rm=TRUE)), keyby =.(obsHalfHour)]
impactSuPostDT <- impactSuPostDT[, . (HhkWh = sum(kWh, na.rm=TRUE)), keyby =.(obsHalfHour)]
impactWiPreDT <- impactWiPreDT[, . (HhkWh = sum(kWh, na.rm=TRUE)), keyby =.(obsHalfHour)]
impactWiPostDT <- impactWiPostDT[, . (HhkWh = sum(kWh, na.rm=TRUE)), keyby =.(obsHalfHour)]
...
myPlot <- ggplot2::ggplot(impactSuPreDT, aes(x=`obsHalfHour`)) +
  #facet_wrap(vars(month))+
  geom_line(aes(y=`HhkWh`, colour = "Pre-Solar"), size =1) +
  geom_line(data = impactSuPostDT, aes( y= `HhkWh`, colour = "Post-Solar"), size=1)+
   labs(x="Time of day",y="Energy consumption (kWh)",title="Aggregated net energy consumption (25 random houses) pre/post solar for second Wednesday in Feb (summer) ", color = "iD")+
  scale_color_manual(values = colors)+
   #scale_fill_discrete(name = "Household iD")+
  theme(text = element_text(family = "Cambria"),
        axis.text.x = element_text(colour = "black"),
...
myPlot <- ggplot2::ggplot(impactWiPreDT, aes(x=`obsHalfHour`)) +
  #facet_wrap(vars(month))+
  geom_line(aes(y=`HhkWh`, colour = "Pre-Solar"), size =1) +
  geom_line(data = impactWiPostDT, aes( y= `HhkWh`, colour = "Post-Solar"), size=1)+
   labs(x="Time of day",y="Energy consumption (kWh)",title="Aggregated net energy consumption (25 random houses) pre/post solar for second Wednesday in August (winter) ", color = "iD")+
  scale_color_manual(values = colors)+
   #scale_fill_discrete(name = "Household iD")+
  theme(text = element_text(family = "Cambria"),
        axis.text.x = element_text(colour = "black"),
